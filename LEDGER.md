# assay ledger

Every verdict, every release, every target. Newest first. Auto-generated on each verdict.

**Totals:** 14 verdicts · ✅ 3 clean · ⚠️ 11 review · 🚨 0 blocked

## Apollo/Reborn

| | Tag | Score | Headline | Judge | Baseline |
|---|---|---|---|---|---|
| ⚠️ `review` | [`v1.15.11_3.7.1`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.7.1) | 7 | Routine bugfix/feature release from known contributors; a few medium-severity co | `claude:sonnet` | `v1.15.11_3.7.0` |
| ✅ `clean` | [`v1.15.11_3.7.0`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.7.0) | 8 | No hard flags from any module | `heuristic-fallback:HTTPError` | `v1.15.11_3.6.0` |
| ⚠️ `review` | [`v1.15.11_3.6.0`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.6.0) | 5 | Hard flag from module(s); manual review needed | `heuristic-fallback:HTTPError` | `v1.15.11_3.5.1` |
| ✅ `clean` | [`v1.15.11_3.5.1`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.5.1) | 8 | No hard flags from any module | `heuristic-fallback:HTTPError` | `v1.15.11_3.5.0` |
| ⚠️ `review` | [`v1.15.11_3.5.0`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.5.0) | 6 | New features added including Gallery View and AI Summaries; verify API key handl | `github-models:openai/gpt-4o-mini` | `v1.15.11_3.4.2` |
| ⚠️ `review` | [`v1.15.11_3.4.2`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.4.2) | 6 | Keychain fixes and new app icon; verify keychain handling | `github-models:openai/gpt-4o-mini` | `v1.15.11_3.4.1` |
| ⚠️ `review` | [`v1.15.11_3.4.1`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.4.1) | 6 | New features and fixes; verify new OAuth interactions and account recovery funct | `github-models:openai/gpt-4o-mini` | `v1.15.11_3.4.0` |
| ⚠️ `review` | [`v1.15.11_3.4.0`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.4.0) | 5 | Hard flag from module(s); manual review needed | `heuristic-fallback:HTTPError` | `v1.15.11_3.3.0` |
| ⚠️ `review` | [`v1.15.11_3.3.0`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.3.0) | 5 | Hard flag from module(s); manual review needed | `heuristic-fallback:HTTPError` | `v1.15.11_3.2.0` |
| ⚠️ `review` | [`v1.15.11_3.2.0`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.2.0) | 5 | Hard flag from module(s); manual review needed | `heuristic-fallback:HTTPError` | `v1.15.11_3.1.1` |
| ✅ `clean` | [`v1.15.11_3.1.1`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.1.1) | 9 | Stability improvements and UI fixes in v1.15.11_3.1.1 | `github-models:openai/gpt-4o-mini` | `v1.15.11_3.1.0` |
| ⚠️ `review` | [`v1.15.11_3.1.0`](https://github.com/Apollo-Reborn/Apollo-Reborn/releases/tag/v1.15.11_3.1.0) | 5 | Hard flag from module(s); manual review needed | `heuristic-fallback:HTTPError` | `v1.15.11_3.0.0` |

## Balackburn/Apollo

| | Tag | Score | Headline | Judge | Baseline |
|---|---|---|---|---|---|
| ⚠️ `review` | [`v1.15.11_2.14.0`](https://github.com/Balackburn/Apollo/releases/tag/v1.15.11_2.14.0) | 6 | New notification backend feature added; verify security of self-hosted backend | `github-models:openai/gpt-4o-mini` | `v1.15.11_2.13.0` |
| ⚠️ `review` | [`v1.15.11_1.15.11_3.0.0`](https://github.com/Balackburn/Apollo/releases/tag/v1.15.11_1.15.11_3.0.0) | 5 | Transition to community-maintained Apollo-Reborn; verify new API key handling | `github-models:openai/gpt-4o-mini` | `v1.15.11_2.14.0` |

---

_Each row links to the upstream release. The corresponding verdict issue lives at https://github.com/epheterson/assay/issues — filter by the `assay:{owner}/{repo}` label for a specific target._
